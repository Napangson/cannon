                        ...
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTablesID">
                                <thead>
                                    <tr>
                                    	<th></th>
                                        <th width="24%">เลขที่ใบสั่งซื้อ</th>
                                        <th width="17%">วันที่</th>
                                        <th width="20%">ราคารวม</th>
                                        <th width="16%">ลูกค้า</th>
                                    </tr>
                                </thead>
                                <tbody>
<?php
include("connectdb.php");
$sql = "select  *  from  `orders`  where member_id='{$_SESSION['mid']}'";
	$rs = mysqli_query($conn, $sql) ;
	while ($data = mysqli_fetch_array($rs, MYSQLI_BOTH)) {
?>
                    <tr>
                      <td><a href="order_detail.php?a=<?=$data['oid'];?>">ดูรายละเอียด</a></td>
    									<td><?=$data['oid'];?></td>
    									<td><?=$data['odate'];?></td>
